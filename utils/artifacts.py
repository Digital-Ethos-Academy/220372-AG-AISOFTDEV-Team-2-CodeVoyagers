...
def save_artifact(
    ...
    *,
    ...
    New: save_artifact(content, "file.txt")
    Legacy: save_artifact(content=..., artifact_type='txt', name='...', description='...')
    ...
    >>> save_artifact(content="hello", artifact_type="txt", name="greeting", description="A greeting")
    PosixPath('.../artifacts/greeting.txt')
    """
    # Handle legacy signature
    if filename is None and artifact_type is not None and name is not None:
        # Legacy mode: construct filename from name and artifact_type
        if not name.endswith(f'.{artifact_type}'):
            filename = f'{name}.{artifact_type}'
        else:
            filename = name
        # In legacy mode, default to overwrite=True for compatibility
        if overwrite is False and artifact_type is not None:
            overwrite = True
    elif filename is None:
        raise ValueError("Either 'filename' or both 'artifact_type' and 'name' must be provided")
